...
const rules = ref({
  ...
  salary: [
    { pattern: /^\d+$/, message: '薪资职能包含数字', trigger: 'blur' }
  ]
});
//表单引用
...
<template>
  ...
  <!-- 数据展示表格 -->
  <div class="container">
    <el-table :data="empList" border style="width: 100%" :header-cell-style="{ backgroundColor: '#f5f7fa' }"
      ...
      <el-table-column label="头像" width="120" align="center">
        <template #default="scope">
          <img :src="scope.row.image" :alt="scope.row.name" height="40px">
        </template>
      </el-table-column>
      ...
      <el-table-column prop="updateTime" label="最后操作时间" :formatter="formatter" width="200" align="center" />
      ...
      <el-row :gutter="3" v-for="(expr, index) in employee.exprList" :key="expr.id">
        ...
        <el-col :span="6">
          <el-form-item size="small" label="公司" :prop="`exprList.${index}.company`" :rules="[
            ...
            { pattern: /^[a-zA-Z0-9\u4e00-\u9fa5]+$/, message: '公司名称只能包含汉字、字母和数字', trigger: 'blur' }
          ]" label-width="60px">
        ...
        <el-col :span="6">
          <el-form-item size="small" label="职位" :prop="`exprList.${index}.job`" :rules="[
            { min: 2, max: 50, message: '职位长度应在2到50个字符之间', trigger: 'blur' },
            { pattern: /^[a-zA-Z0-9\u4e00-\u9fa5]+$/, message: '职位只能包含汉字、字母和数字', trigger: 'blur' }
          ]" label-width="60px">
